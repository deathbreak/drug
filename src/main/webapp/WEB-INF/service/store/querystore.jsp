		...
		<c:if test="${queryresult.list.size()==0}">
			<div class="row">
				<div class="page-header">
					<h3 class="text-center">没有查询到符合条件的药品信息！<a href="<%=basePath%>store">返回</a></h3>
					<div class="text-center"><small>若用条形码查询没有结果,可尝试用名称查询,若还是没有,则无此药品信息</small></div>
				</div>
			</div>
		</c:if>
		<c:if test="${queryresult.list.size()>0}">
			...
			<div class="row">
				...
				<div class="col-md-6">
					...
					<ul class="pagination">
						<li><a href="<%=basePath%>querystore?pn=1&querystore=${querystring }">首页</a></li>
						<c:if test="${queryresult.hasPreviousPage }">
							<li><a href="<%=basePath%>querystore?pn=${queryresult.pageNum - 1}&querystore=${querystring }"
						...
						<c:forEach items="${queryresult.navigatepageNums }" var="page_Num">
							...
							<c:if test="${page_Num != queryresult.pageNum}">
								<li><a href="<%=basePath%>querystore?pn=${page_Num }&querystore=${querystring }">${page_Num }</a></li>
							</c:if>
						</c:forEach>
						...
						<li><a href="<%=basePath%>querystore?pn=${queryresult.pages}&querystore=${querystring }">末页</a></li>
						<!--  -->
						<li>
							<div class="btn-group dropup">
								...
								<ul class="dropdown-menu">
									<c:forEach var="x" begin="1" end="${queryresult.pages}"
										step="10">
										<li><a
											href="<%=basePath%>querystore?pn=${x}&querystore=${querystring }">跳转至第${x }页</a></li>
									</c:forEach>
								</ul>
							</div>
						</li>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form action="<%=basePath%>updatestore" method="post">
						...
						<div class="form-group">
							<label for="inputbp" class="control-label">进价:</label><span class="forclean" style="color:red" id="span_bp"></span>
							<input type="number" step="0.0001" class="form-control input-sm" id="inputbp" name="beginprice" value="" placeholder="如果不知道进价,就输入0">
						</div>
						<div class="form-group">
							<label for="inputpri" class="control-label">销售价格:</label><span class="forclean" style="color:red" id="span_pri"></span>
							<input type="number" step="0.0001" class="form-control input-sm" id="inputpri" name="price" value="">
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							<label for="inputph" class="control-label">批号:</label><span class="forclean" style="color:red" id="span_ph"></span>
							<input type="text" class="form-control input-sm" id="inputph" name="pihao" value="">
						</div>						
						<div class="form-group">
							<label for="inputkc" class="control-label">库存:</label><span class="forclean" style="color:red" id="span_kc"></span>	
	...
	<div class="modal fade" id="myModal2" tabindex="-1" role="dialog">
		<div class="modal-dialog modal-sm" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form action="<%=basePath%>deletestore" method="post">
