...
<div class="container-fluid">
    ...
    <c:if test="${empty drugstoreinfo }">
        <div class="row">
            <div class="page-header">
                <h3 class="text-center">没有药品信息！<a href="<%=basePath%>addstore">返回</a></h3>
            </div>
        </div>
    </c:if>
    <c:if test="${not empty drugstoreinfo }">
        <div class="panel panel-info">
            <div class="panel-heading">
                <h3 class="panel-title">添加库存药品信息</h3>
            </div>
            <form action="<%=basePath%>addtostore" method="post">
                <table class="table">
                    <tr>
                        <td>药品信息:</td>
                        <td><input type="text" class="form-control" name="drugname" value="${drugstoreinfo.drugname }"
                                   readonly></td>
                        <td>单位:</td>
                        <td><input type="text" class="form-control" name="unit" value="${drugstoreinfo.unit }" readonly>
                        </td>
                        <td>规格:</td>
                        <td><input type="text" class="form-control" name="guige" value="${drugstoreinfo.guige }"
                                   readonly></td>
                    </tr>
                    <tr>
                        <td>生产厂商:</td>
                        <td><input type="text" class="form-control" name="changshang"
                                   value="${drugstoreinfo.changshang }" readonly></td>
                        <td>生产日期:</td>
                        <td><input type="text" class="form-control picker" id="inputbd" name="begindate" value=""
                                   readonly></td>
                        <td>有效期至:</td>
                        <td><input type="text" class="form-control picker" id="inputda" name="date" value="" readonly>
                        </td>
                    </tr>
                    <tr>
                        <td>备注:</td>
                        <td><input type="text" class="form-control" name="beizhu" value="${drugstoreinfo.beizhu }"
                                   readonly></td>
                        <td>进价:</td>
                    ...
                    <tr>
                        <td>药品位置:</td>
                        <td><input type="text" class="form-control" name="location" value="${drugstoreinfo.location }"
                                   readonly></td>
                        <td>批号:</td>
                        <td><input type="text" id="inputph" class="form-control" name="pihao" value=""></td>
                        <td>库存:</td>
                        <td><input type="number" id="inputkc" class="form-control" name="count" value=""></td>
                    </tr>
                    <tr>
                        <td>条形码:</td>
                        <td><input type="text" class="form-control" name="tiaoxingma"
                                   value="${drugstoreinfo.tiaoxingma }" readonly></td>
                        <td></td>
                        <td><span id="tips" style="color:red"></span></td>
                        <td></td>
                        <td>
                            <button type="submit" class="btn btn-primary btn-block" id="btn-submit">添加到库存</button>
                        </td>
                    </tr>
                </table>
            </form>
